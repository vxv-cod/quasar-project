...
<br><br>pagesNumber {{ pagesNumber }} -->
  ...
  <div class="contener" style="">
    ...
    <div class="panel top">
      <div
        class=""
        style="min-width: 35px; border-right: 1px solid white"
      ...
      >
        ...
        <q-btn
          ...
          @click="
            (showEditIcon = !showEditIcon),
              !showEditIcon ? saveEditData() : null
          "
        ...
        >
          <q-tooltip
            ...
            anchor="top middle"
            self="bottom middle"
            >Редактировать таблицу</q-tooltip
          >
          <q-tooltip v-else anchor="top middle" self="bottom middle"
            >Сохранить таблицу</q-tooltip
          >
        </q-btn>
      ...
      <table>
          ...
          <tr v-for="rowH in countRowsHeder" :key="rowH">
           ...
           -->
            <th
              ...
              :ref="
                col.fixed && iconShowHeds(col, rowH) ? 'fixRef' : 'notfixRef'
              "
              :rowspan="
                countRowsHeder.length == 2 &&
                rowH === 1 &&
                col.mergeParentName &&
                col.rowspan === true
                  ? 1
              ...
              "
              :colspan="
                rowH === 1 ? computedColsParents[col.mergeParentName] : 1
              "
              :style="[iconShowHeds(col, rowH) ? collFix(col, 5) : null]"
              v-show="showCelsHed(col, rowH)"
            >
              <div class="thHeaderBox">
                ...
                <div style="display: flex">
                  ...
                  <menuFilter
                    ...
                    :colorBtn="
                      Object.keys(filterColumns).includes(col.name) &&
                      col.name != []
                        ? `text-amber`
                        : null
                    "
                  ...
                  <q-btn
                    ...
                    @click="
                      rows = customSort(col);
                      !sortGeneral ? (col.countSorted = 0) : null;
                    "
                    :class="[
        ...
        <tbody>
          <tr
            ...
            :style="[rowsPagsInd?.includes(row.idRow) ? null : `display: none`]"
          >
            <td
              v-if="multiSelectRows"
              :style="[
                `position: sticky; left: 0; z-index: 4; background-color: white`,
                selected.includes(row.idRow)
            ...
            <td
              ...
              :style="[
                ...
                selected.includes(row.idRow)
                  ? 'background-color: #cfd8dc'
                  : null,
                selected.includes(row.idRow) && col.fixed === true
...
const loadRowsHeaders = store.state.loadRowsHeaders
// const counter = computed(() => store.state.counter)
// const fff = computed(() => {
//   return [...mapState(['counter', 'loadRowsHeaders'])]
// })
// const ggg = computed(() => {
...
function pluss() {
  store.commit("increment", store.state.counter + 1);
}
...
const mergColsBool = computed(() =>
  loadHeaders.some((e) => Object.keys(e).includes("columns"))
);
...
zeroHeaders.forEach((e, i) => {
  ...
  e["fixed"] = false;
  e["label"].includes("%")
    ? (e["sort"] = (a, b) => parseInt(a, 10) - parseInt(b, 10))
    : null;
  (e["idCol"] = i), (e["edit"] = e.edit === false ? false : true);
});
...
const columnKeys = reactive(headers.value.map((e) => e.name)).filter(
  (e) => e !== "idRow"
);
const columnLabels = reactive(headers.value.map((e) => e.label)).filter(
  (e) => e !== "idRow"
);
...
const visibleColumns = ref(
  zeroHeaders.map((e) => e.name).filter((e) => e !== "idRow")
);
...
function showCelsHed(col, rowH) {
  let ParentNameBool = Object.keys(computedColsParents.value)?.includes(
    col.mergeParentName
  );
  if (visibleColumns.value.includes(col.name)) {
    if (countRowsHeder.value.length == 2) {
      if (rowH === 1) {
        return !col.numberMergeCell || col.numberMergeCell === 1 ? true : false;
      }
      if (rowH === 2) {
        return col.mergeParentName ? true : false;
      }
    } else {
      return true;
    }
  } else {
    return rowH === 1 && col.numberMergeCell === 1 && ParentNameBool
      ? true
      : false;
  }
}
...
function iconShowHeds(col, rowH) {
  if (countRowsHeder.value.length == 2) {
    return (rowH === 1 && !col.mergeParentName) ||
      (rowH === 2 && col.mergeParentName)
      ? true
      : false;
  } else {
    return true;
...
const columnsIndexRow = ref({});
columnKeys.map(
  (col) => (columnsIndexRow.value[col] = rows.value.map((e) => e["idRow"]))
);
...
watch(
  ...
  (item) => {
    ...
    if (Object.values(filterColumns.value).length >= 1) {
      ...
      defaultColumns.value[itemKey].forEach((e, i) => {
        if (itemVals.includes(e)) {
          idxListfilter.value.push(i);
        }
      });
      ...
      Object.entries(defaultColumns.value).forEach(([key, vals]) => {
        ...
        if (key !== itemKey) {
          ...
          if (key !== key0) {
            delete filterColumns.value[key];
          }
        } else {
          selectAllFil.value[key] = itemVals;
        }
      });
  ...
  (bool) => {
    if (bool === false) {
      if (Object.keys(filterColumns.value).length !== 0) {
        ...
        Object.keys(filterColumns.value)
          .reverse()
          .map(
            (e) =>
...
const selectedRowsChanged = (item) => {
  if (multiSelectRows.value === true) {
    if (selected.value.includes(item) == false) {
...
function toggleAll() {
  if (selected.value.length) {
    ...
    selected.value = rows.value.map((e) => e.idRow);
  }
}
...
const selectedAll = computed(() => {
  ...
  if (
    ...
    selected.value.length !== rows.value.length
  ) {
    bool = null;
  }
...
const IndentsTD = (side) => {
  ...
  if (fixRef.value !== null) {
    columnKeysList.forEach((x) => {
...
function sumForColFUN(arr) {
  if (arr.some((tag) => isNaN(tag))) {
    return null;
  } else {
    let xxx = arr.reduce((sum, current) => sum + current, 0);
    if (Number.isInteger(xxx)) {
      return xxx;
    } else {
